...
def bark():
  ...
  """
  dog_name = input("What is the dog's name? ").capitalize()
  dog_age = int(input("What is the dog's age? "))
  dog_breed = input("What is the dog's breed? ").capitalize()
...
def bark_with_validation():
  ...
  if not dog_name.isalpha() or len(dog_name) < 2:
    print("Name error!")
  elif dog_age < 0 or dog_age > 15:
    print("Age error!")
  elif dog_breed not in ["Schnauzer", "Terrier", "Poodle", "Mastiff"]:
    print("Breed error!")
  else:
    print(f'{dog_name}, the {dog_age} year old {dog_breed}, says, "Woof!"')
